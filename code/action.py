...
attack_success_no_sound = [(.5, 0.5), (1, 1), (0.5, 2), (0.5, 1)]
attack_success_sound = [(0, 0.5), (0, 1), (0, 2), (0.5, 1)]
attack_success_soundnfish = [(.5, 0.5), (1.5, 1), (1, 2), (0.5, 1)]
...
class ActionPlanner():
    ...
    def get_random_action_success(self):
        ...
        success = np.random.normal(success_mean, success_var) > 1.5 
    ...
    def get_planned_action_success(self, prey):
        ...
        if len(self.action_x_train) < 10:
            ...
            self.action_x_train.append((self.event_count, self.prev_action_index, action_index))
            self.action_y_train.append(success)
            self.prev_action_index = action_index
            
        else:
            # Consider past actions...
            training_x = np.array(self.action_x_train)
            ...
            for i in range(0, len(attack_success_no_sound)):
                o = trained_model.score(np.array((self.event_count + 1, self.prev_action_index, i)),\
                    np.reshape(np.array([True]), (1,1)))
                opinions.append(o)
