...
attack_success_no_sound = [(0, 1), (0, 8), (0, 7), (0, 6)] # action success array after not any sound
attack_success_sound =  [(0, 1), (0, 5.5), (0, 6), (0, 5)] # action success array after making sound around evesdropping mammals
attack_success_soundnfish = [(0, 1), (0, 10), (0, 8), (0, 6)] # action success array after making sound around fish
...
class ActionPlanner():
    ...
    def get_random_action_success(self):
        ...
        success = -1
        if np.random.uniform(success_mean, success_var) >= SUCCESS_THRESHOLD:
            success = 1
    ...
    def get_planned_action_success(self, prey):
        ...
        if len(self.action_x_train) < 10: 
            ...
            if len(self.evaluation_vector) ==0:
                self.evaluation_vector = np.array([a for a in prob_opinions])
            else:
            ...
            if self.prev_action_index != 0:
                print "prev action is not zero:",self.prev_action_index
                success_mean, success_var = attack_success_no_sound[action_index]
            else:
